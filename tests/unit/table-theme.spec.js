...
describe('浅色科技表格视觉契约', () => {
  test('声明方案 C 的表格层级、动态效果和状态语义选择器', () => {
    ...
    expect(stylesheet).toContain('.tech-tone-cyan')
    expect(stylesheet).toContain('.tech-tone-mint')
    expect(stylesheet).toContain('.tech-tone-violet')
    expect(stylesheet).toContain('.tech-tone-amber')
    expect(stylesheet).toContain('.tech-tone-coral')
    expect(stylesheet).toContain('.tech-table-action.el-button--danger')
    expect(stylesheet).toContain('flex-wrap: nowrap')
    expect(stylesheet).toContain('overflow-x: auto')
    expect(stylesheet).toContain('min-width: 360px')
    expect(stylesheet).toContain(':has(.tech-table-action)')
