<template>
  <div class="app-container" style="min-height: 700px;">
    ...
    <el-dialog title="增加类型" :visible.sync="showAdd" width="700px" append-to-body>
      <el-row :gutter="20">
        <el-col :span="12">
          <el-form ref="addForm" :model="addData" :rules="addRules" label-width="80px">
            <el-form-item label="通道编码" prop="device_id">
        ...
        <el-col :span="12">
          <el-form ref="addForm" :model="addData" :rules="addRules" label-width="80px">
            ...
            <el-form-item label="是否使用" prop="is_handle">
              <el-switch v-model="addData.is_handle" />
            </el-form-item>
          </el-form>
        </el-col>
      </el-row>
  ...
  methods: {
    ...
    async comfirmAdd() {
      try {
        ...
        if (Number(response.code) !== 200) throw new Error(response.msg)
        this.$message({
          message: '添加成功',
          type: 'success'
        })
